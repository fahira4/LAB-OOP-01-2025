...
public class App extends Application {
   ...
   private void resizeImages(double containerWidth) {
    ...
    for (Node node : postsContainer.getChildren()) {
        if (node instanceof StackPane) {
            ...
            pane.setClip(new Rectangle(newWidth, newHeight));
            ...
            // Update ukuran label caption
            Label captionLabel = (Label) pane.getChildren().get(1);
            captionLabel.setMaxWidth(newWidth);
            captionLabel.setPrefHeight(newHeight * 0.3); // 30% dari tinggi gambar
        }
    }
}
   ...
   private void updatePostsDisplay() {
    ...
    for (Post post : posts) {
        ...
        captionLabel.setStyle("-fx-text-fill: white; -fx-font-weight: bold; -fx-font-size: 14px;");
        captionLabel.setMaxWidth(Double.MAX_VALUE);
        captionLabel.setMaxHeight(Double.MAX_VALUE);
        
        // StackPane untuk menumpuk gambar dan caption
        StackPane imageContainer = new StackPane(postView, captionLabel);
        imageContainer.setUserData(post);
        imageContainer.setStyle("-fx-effect: dropshadow(three-pass-box, rgba(0,0,0,0.2), 5, 0, 0, 0); -fx-cursor: hand;");
        
        // Atur agar caption awalnya tidak terlihat
        captionLabel.setVisible(false);
        captionLabel.setManaged(false);
        
        // Efek hover untuk menampilkan caption
        imageContainer.setOnMouseEntered(e -> {
            captionLabel.setVisible(true);
            captionLabel.setManaged(true);
            // Background semi-transparan
            captionLabel.setStyle("-fx-background-color: rgba(0,0,0,0.7); -fx-text-fill: white; -fx-font-weight: bold; -fx-font-size: 14px; -fx-padding: 10px;");
